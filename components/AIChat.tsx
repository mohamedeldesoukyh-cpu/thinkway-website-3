...
{!open && (
  <motion.div
    ...
    className="fixed bottom-8 right-10 z-[9999] flex items-center gap-4"
  >
    {/* Text Pill */}
    <button
      onClick={() => setOpen(true)}
      className="h-16 px-8 rounded-full bg-white/90 backdrop-blur-xl border border-white/40 shadow-[0_0_40px_rgba(21,53,194,0.18)] text-[15px] font-semibold text-[#111] hover:scale-[1.03] hover:shadow-[0_0_60px_rgba(21,53,194,0.35)] transition-all duration-500"
    >
      Try Thinkway AI
    </button>
    ...
    <motion.button
      ...
      onClick={() => setOpen(true)}
      className="w-16 h-16 rounded-full bg-[#1535C2] shadow-[0_0_45px_rgba(21,53,194,0.55)] flex items-center justify-center text-white text-2xl hover:scale-110 transition-all duration-500"
    >
      ✦
    </motion.button>
  </motion.div>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="fixed bottom-8 right-10 z-[9999] w-[420px] h-[650px] bg-white rounded-[32px] shadow-[0_20px_80px_rgba(0,0,0,0.18)] border border-[#ececec] overflow-hidden flex flex-col backdrop-blur-xl"
          >
           {/* Header */}
