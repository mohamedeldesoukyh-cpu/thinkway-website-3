...
export default function Packages() {
  ...
  return (
    <section id="packages" ref={ref} className="section-padding bg-[#080808] relative overflow-hidden">
      {/* Accent */}
      <div
        className="absolute top-0 left-1/2 -translate-x-1/2 w-px h-40 pointer-events-none"
        style={{ background: "linear-gradient(to bottom, #c0392b, transparent)" }}
      />
      ...
      <div className="container-custom">
        ...
        >
          ...
          >
            PRICING &amp;
            <br />
            <span className="text-[#c0392b]">PACKAGES</span>
          </h2>
          <p className="text-[10px] text-[#444] tracking-[0.12em] uppercase mt-6 leading-[2.2] max-w-md mx-auto">
            Every package is tailored to your brand&apos;s goals and budget. All pricing is bespoke — these tiers are frameworks, not ceilings.
          </p>
        </motion.div>
            ...
            <motion.div
              ...
              style={{
                background: pkg.highlight
                  ...
                  ? "rgba(192,57,43,0.03)"
                  : "#080808",
                border: pkg.highlight ? "1px solid rgba(192,57,43,0.3)" : "none",
              }}
            >
              {/* Popular badge */}
              {pkg.highlight && (
              ...
              <div className="mb-8">
                ...
                <div className="text-[9px] tracking-[0.15em] text-[#444] uppercase mt-2">{pkg.note}</div>
                <div className="text-[10px] text-[#333] mt-2">{pkg.tagline}</div>
              </div>

              {/* Feature list */}
              <div className="space-y-0 border-t border-[#111] pt-6">
                {pkg.features.map((f) => (
                  <div key={f.label} className="flex justify-between items-center py-3 border-b border-[#0d0d0d]">
                    <span className="text-[9px] tracking-[0.12em] uppercase text-[#444]">{f.label}</span>
                    <span className="text-[9px] tracking-[0.08em] text-white font-medium">{f.val}</span>
                  </div>
                ))}
              </div>
              ...
              <button
                className={`w-full mt-8 py-4 text-[10px] tracking-[0.18em] uppercase font-medium transition-all duration-300 ${
                  pkg.highlight
                    ...
                    : "border border-[#1a1a1a] text-[#666] hover:border-[#c0392b] hover:text-white"
                }`}
        ...
        <motion.p
          ...
          className="text-center text-[9px] text-[#333] tracking-[0.18em] uppercase mt-10"
        >
